...
public class FreemarkerServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (request.getRequestURI().isEmpty() || request.getRequestURI().equals("/")) {
            template = freemarkerConfiguration.getTemplate("HomePage.ftlh");
        } else {
            try {
        ...
        Map<String, Object> data = getData(request, flag);
    ...
    private Map<String, Object> getData(HttpServletRequest request, boolean flag) {
        Map<String, Object> data = new HashMap<>();

        for (Map.Entry<String, String[]> e : request.getParameterMap().entrySet()) {
            if (e.getKey().contains("_id") && check_is_long(e.getValue()[0])) {
        ...
        if (!flag) {
            ...
            if (!uri.isEmpty() && !uri.equals("/")) {
                data.put("cur_menu", uri);
            } else {
                data.put("cur_menu", "/HomePage");
            }
        } else {
            data.put("cur_menu", "/error");
        }
    ...
    private static boolean check_is_long(String num) {
        try {
            Long.parseLong(num);
            return true;
