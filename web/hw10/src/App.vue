    ...
    beforeCreate() {
        ...
        this.$root.$on("onRegister", (login, name) => {
          ...
          if (!login || login.length < 3 || login.length > 16) {
            this.$root.$emit("onRegisterValidationError", "Login length must be from 3 characters to 16");
          } else if (!name || name.length < 1 || name.length > 32) {
            this.$root.$emit("onRegisterValidationError", "Name length must be from 1 characters to 32");
